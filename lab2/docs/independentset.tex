...
$1$, $v$, is in the $MIS$. The neighbour of $v$ has $degree > 1$, otherwise one
of them but not both may be in the $MSI$ and it wont affect the result which
one. If the naighbour of $v$ is in the $MSI$, $v$ and at least one more vertex
is not in the $MIS$. If $v$ is in the $MIS$ only its neighbour is not in the
set and its neighbours neighbours may be, resulting in an identical graph
except for at least one additional vertex if $v$ is in the $MSI$. Hence it must
be at least as good to include any vertex with $degree == 1$.
